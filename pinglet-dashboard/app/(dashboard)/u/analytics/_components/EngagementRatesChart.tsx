...
interface Props {
  initialData: Array<{ time: string; clickRate: number; dropRate: number; closeRate: number }> | null
}

export function EngagementRatesChart({ initialData }: Props) {
  ...
  const handleFilterChange = useCallback(async (filter: string) => {
    ...
    setLoading(true)
    try {
      const res = await API.getEngagementRates(filter)
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div>
          ...
          <CardDescription>Track user engagement metrics as percentages</CardDescription>
        </div>
