export const eventTriggers = [
    { value: "notification.sent", label: "Notification Sent" },
    { value: "notification.dropped", label: "Notification Dropped" },
    { value: "notification.closed", label: "Notification Closed" },
    { value: "notification.failed", label: "Notification Failed" },
    { value: "notification.clicked", label: "Notification Clicked" },
    { value: "notification.queued", label: "Notification Queued" },
    { value: "notification.delivered", label: "Notification Delivered" },
    { value: "notification.dismissed", label: "Notification Dismissed" },
    { value: "user.subscribed", label: "User Subscribed" },
    { value: "user.unsubscribed", label: "User Unsubscribed" },
    { value: "project.created", label: "Project Created" },
    { value: "domain.verified", label: "Domain Verified" },
]