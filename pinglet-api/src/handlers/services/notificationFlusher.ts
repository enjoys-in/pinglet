...
import type { ANALYTICS_EVENTS } from "@/utils/services/kafka/topics";
...
class NotificationFlusherService {
		...
		for (const baseCounterKey of baseCounterKeys) {
			...
			try {
				...
				const insertValues = {
					...
					total_request: Number.parseInt(counters.request || "0", 10),
					total_sent: Number.parseInt(counters.sent || "0", 10),
					total_clicked: Number.parseInt(counters.clicked || "0", 10),
					total_failed: Number.parseInt(counters.failed || "0", 10),
					total_dropped: Number.parseInt(counters.dropped || "0", 10),
					total_closed: Number.parseInt(counters.closed || "0", 10),
				};
				...
				console.error(
					`[FlushCounter] Failed to flush counter for project ${projectId}:`,
					err,
				);
				await this.redis.rPush(
					REDIS_KEYS_NAME.ANALYTICS_RETRY_DELTA,
					tmpCounterKey,
				);
			...
			try {
				await this.notificationLogRepo.manager.transaction(async (tx) => {
					const logs = rawLogs.map((raw) => {
						const { notificationId, event, timestamp, type, ...metadata } =
							JSON.parse(raw);
				...
				console.error(
					`[Buffer Counter] Failed to flush counter for project ${projectId}:`,
					err,
				);
				await this.redis.rPush(
					REDIS_KEYS_NAME.ANALYTICS_RETRY_BUFFER,
					tmpBufferKey,
				);
	...
	async fetchAllRegisteredProjects(): Promise<void> {
		const baseCounterKey = REDIS_KEYS_NAME.ANALYTICS_DELTA.replace(
			"projectId",
			"*",
		);
		const baseBufferKey = REDIS_KEYS_NAME.ANALYTICS_BUFFER.replace(
			"projectId",
			"*",
		);
...
export const notificationFlusherService = new NotificationFlusherService(
