		...
		init({ endpoint, expectedDomains = [], projectIds = [] }) {
			...
			if (expectedDomains.length === 0) {
				this._showPopup(
					...
					"Please add at least one domain to your Pinglet configuration.",
				);
				return;
			}
			...
			projectIds.forEach((pid) => {
				const source = new EventSource(
					`${endpoint}?projectId=${encodeURIComponent(pid)}`,
				);
				source.onmessage = (e) => {
					try {
						...
						// playNotificationSound({ play: globalConfig.sound.play, src:globalConfig.sound.sound });
